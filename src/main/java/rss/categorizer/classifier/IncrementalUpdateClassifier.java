	...
	private static Double learning_rate_index = Math.tan(Math.PI/32);
	    ...
	    public static void main(String[] args) {
			...
			for(Tuple3<Long, Integer, Boolean> pred : predictions) {
				...
				total_count++;
				acc.add((total_correct_count/(double) total_count) * 100);
				lr.add(local_learning_rate.get((int) Math.floor(ctr2 / (double) incremental_update_freq)));
		...
		System.out.println("acc: " + acc.size() + acc);
			System.out.println(lr.size() + " " +lr);
	    }
