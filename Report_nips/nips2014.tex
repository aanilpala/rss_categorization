...
In the time of three weeks we received 3659 data points. This rather small amount of data restriced us in choosing training and test set size since we are oberserving the classification behavior over time. For each approach our initial training phase contains 600 data instances which equates to the first three days of streaming news feeds. 

Our first approach - the offline model - is used as a reference model. After the training phase we have chosen three disjoint test sets with 1000 datapoints each as seen in the graphic below to retrieve possible performance changes.  Our size of training and test set differes from other test setups which usually take a 50:50 to 80:20 ratio for set sizes.  Of course the classifier performs better the more training data is available, but since we are using sliding window approaches for the online classification with limited capacity we  have chosen this split for compatibility in the comparison with the different scenarios and  with the larger testset to get a more accurate error estimation.  Due to this experimental setup we are not expecting a very high perfomance accurancy in general but to detect changes in performance which could indicate some sort of concept drift.
\begin{figure}[htbp]
  \centering
...
As seen here the retraining of the error-triggered approach takes place two more times at test items 600 and 900 which is the sanity window size. It  followes by a stable model for over 1300 incoming test items and is retrained again two times. In this visualisation we have to consider that the test sets of the error-triggered models are much smaller than the bruteforce ones. However, the performance of the error-triggered models are not neccessarily much better than the bruteforce models except in the last interval starting at test items 2600. When applying smaller sanity windows constant retraining is performed due to the smaller test sets. The stable phase of the error-triggered approach starts at test item 900 just 100 data points before the bruteforce retraining is done but decreases shortly before 1000 nearly to the threshold. This drop is displayed as a less accumulative interval accuracy within this stable phase than the bruteforce approach. This means retraining the model in this point of time around receiving 1000 test items leads to a better prediction for a longer period. This might be caused by newsfeeds which are related to a certain topic more often and are included in training and test set. Out of this phase topic distribution might be broader and a classifier with 600 training items and 5 classes does just not perfom over 63\% especially when data points are as sparse as those we used. For a profound analysis accuracy is also not the way to go.  

%TODO: Anil incremental evaluation
...
\end{figure}
The cumulative accuracy over all intervals shows that error-triggered and batch performance is nearly the same due to the oscillations the error-triggered models produce, which are summed up relativize the overall accuracy. The way we implemented the different window techniques and with measurement we are using we have to derive that retraining very often does not neccessarily lead to better performance. In our case we had been lucky with choosing the intervals for bruteforce technique which reflexts as well in the overall accuracy. There might be some sort of concept drift but this needs further exploration and more testing with different paramters and measure methods which was not doable in the given time.
...
According to our assuptions concept drift might have been detected over the three weeks but with the given setup we can not identify which sort of concept drift nor when exactly. We did oberseve changes in accuracy performance which we relate to changes in the probabilities of the classifier and to concept drift. We derive that for clearer detection and adaption of concept drift profound measures of changes in probabilies are essential and a longer period of time might also be neccassary. 
Furthermore retraining does improve the accuracy if it is done reasonable at a well chosen point in time respectivly with a qualified parameters. Often retraining with a small window as a forgetting memory does not qualify to adapt concept drift on basis of accuracy within sparse data. 
%TODO:Anil statement to incremtal, check if ok: 
A flexible forgetting memory as the incremental technique could therefore be a better option. A training phase of 600 data points was chosen too small, obiously a classifier with 5 classes and sparse data might need more training. To reduce sparsety of data further preprocessing could be a handy step as well as handle bias. A variable windowsize sould also be considered since this is crucial for training or the combination of classifiers as ensemble learners. 
For real world problems in context of large scale data a good starting point of thinking might be rather a suitable data architecture like Lambda architecture which includes batch and real-time layer and set up a classifier on top considering the available data but this clearly goes beyond the scope of this project. 
